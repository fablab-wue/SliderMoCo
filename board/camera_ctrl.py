...
# Open-collector, active-low — same drive as SliderMC PIN_CAMERA_CTRL:
# assert = OUTPUT LOW; idle = INPUT_PULLUP.
...
class CameraCtrl:
    """Open-collector active-low camera trigger on PIN_CAMERA_CTRL."""
    ...
    def __init__(self, pin=None, pulse_ms=None):
        ...
        if pin is None or Pin is None:
            ...
            return
        try:
            self._pin = Pin(int(pin), Pin.IN, Pin.PULL_UP)
            dbg(3, "camera_ctrl GP%s OC" % int(pin))
    ...
    def pulse(self, ms=None):
        """Sink low for pulse_ms (blocking)."""
